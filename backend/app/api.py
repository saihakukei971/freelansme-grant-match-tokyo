...
async def get_stats(
    ...
    """補助金統計情報API"""
    # 総数
    total_count = db.exec(select(Subsidy)).count()
    ...
    # 交付団体別の数
    org_query = select(Subsidy.organization, count=func.count(Subsidy.id)).\
        group_by(Subsidy.organization)
    organizations = db.exec(org_query).all()
